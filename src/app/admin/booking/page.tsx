...
// Dynamically import BookingCalendar to prevent SSR issues with FullCalendar plugins
...
const BookingManagementPage = () => {
  ...
  useEffect(() => {
    const fetchCalendarEvents = async () => {
      try {
        const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000';
        const res = await fetch(`${apiUrl}/api/admin/bookings/summary`);
        if (res.ok) {
          const data = await res.json();
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("คุณแน่ใจหรือไม่ที่จะลบ/ยกเลิกรายการจองนี้?")) return;
    try {
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000';
          ...
          {viewMode === "calendar" && (
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 mb-8">
              <h2 className="text-xl font-bold text-slate-900 mb-6 tracking-tight">
                ปฏิทินสรุปตารางเวลาจองห้องประชุมทั้งหมด
              </h2>
